...
st.write("U-Net is a convolutional neural network designed for semantic segmentation, featuring a U-shaped architecture that includes a contracting path for context capture and an expansive path for precise output reconstruction. By utilizing skip connections to combine features from the encoder and decoder, U-Net effectively preserves spatial information, making it particularly well-suited for tasks like biomedical image segmentation where accurate pixel-level predictions are essential.")
st.subheader("Model architecture")
image_file = "model_unet.png"  
image = Image.open(image_file)
st.image(image, caption='Displayed Image.', use_column_width=True)

image_file = "graph_unet.png"
st.subheader("Loss,Accuracy Graphs")
image = Image.open(image_file)
st.image(image, caption='Displayed Image.', use_column_width=True)
st.write("accuracy: 0.6663 - loss: 0.2821")
st.write("Upload an image and perform segmentation")

image_file = "image_unet.png"
image = Image.open(image_file)
st.image(image, caption='Displayed Image.', use_column_width=True)
